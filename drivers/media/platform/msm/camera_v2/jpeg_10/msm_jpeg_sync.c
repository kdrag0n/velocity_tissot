...
inline static void msm_jpeg_q_init(char const *name, struct msm_jpeg_q *q_p)
{
...
inline static void *msm_jpeg_q_out(struct msm_jpeg_q *q_p)
{
	unsigned long flags;
...
inline static int msm_jpeg_q_in(struct msm_jpeg_q *q_p, void *data)
{
	unsigned long flags;
...
inline static int msm_jpeg_q_in_buf(struct msm_jpeg_q *q_p,
...
inline static int msm_jpeg_q_wait(struct msm_jpeg_q *q_p)
{
...
inline static int msm_jpeg_q_wakeup(struct msm_jpeg_q *q_p)
{
	JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
	wake_up(&q_p->wait);
	return 0;
...
inline static int msm_jpeg_q_unblock(struct msm_jpeg_q *q_p)
{
	JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
	q_p->unblck = 1;
	wake_up(&q_p->wait);
...
inline static void msm_jpeg_outbuf_q_cleanup(struct msm_jpeg_device *pgmn_dev,
...
inline static void msm_jpeg_q_cleanup(struct msm_jpeg_q *q_p)
{
	void *data;
	JPEG_DBG("%s:%d] %s\n", __func__, __LINE__, q_p->name);
	do {
