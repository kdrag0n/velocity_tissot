...
static enum ipa_client_type ipa_rm_resource_prod[MAX_BAMS] = {
	[CI_CTRL] = (enum ipa_client_type) IPA_RM_RESOURCE_USB_PROD,
	[HSIC_CTRL]  = (enum ipa_client_type) IPA_RM_RESOURCE_HSIC_PROD,
	[DWC3_CTRL] = (enum ipa_client_type) IPA_RM_RESOURCE_USB_PROD,
};

static enum ipa_client_type ipa_rm_resource_cons[MAX_BAMS] = {
	[CI_CTRL] = (enum ipa_client_type) IPA_RM_RESOURCE_USB_CONS,
	[HSIC_CTRL]  = (enum ipa_client_type) IPA_RM_RESOURCE_HSIC_CONS,
	[DWC3_CTRL] = (enum ipa_client_type) IPA_RM_RESOURCE_USB_CONS,
};
...
static void wait_for_prod_release(enum usb_ctrl cur_bam)
{
	...
	log_event_dbg("%s: Releasing %s_PROD\n", __func__,
				bam_enable_strings[cur_bam]);
	ret = ipa_rm_release_resource((enum ipa_rm_resource_name) ipa_rm_resource_prod[cur_bam]);
	if (!ret) {
