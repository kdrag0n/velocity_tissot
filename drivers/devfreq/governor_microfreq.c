 ...
 */
u64 inline suspend_time_ms(void)
{
	if (suspend_start == 0)
		return 0;

	u64 suspend_sampling_time = (u64)ktime_to_ms(ktime_get());
	u64 time_diff = suspend_sampling_time - suspend_start;
	/* Update the suspend_start sample again */
	suspend_start = suspend_sampling_time;
	return time_diff;
}

static inline ssize_t gpu_load_show(struct device *dev,
 ...
 */
static inline ssize_t suspend_time_show(struct device *dev,
	struct device_attribute *attr,
	char *buf)
{
	 ...
	 * since last read.
	 */
	u64 time_diff = suspend_time_ms() + suspend_time;
	suspend_time = 0;
...
void inline compute_work_load(struct devfreq_dev_status *stats,
...
static inline int __secure_tz_reset_entry2(unsigned int *scm_data, u32 size_scm_data,
					bool is_64)
{
	int ret;
...
static inline int __secure_tz_update_entry3(unsigned int *scm_data, u32 size_scm_data,
...
static inline int tz_init_ca(struct devfreq_msm_adreno_tz_data *priv)
{
	...
	u8 *tz_buf = kzalloc(PAGE_ALIGN(sizeof(tz_ca_data)), GFP_KERNEL);
	if (!tz_buf)
		return -ENOMEM;
...
static inline int tz_init(struct devfreq_msm_adreno_tz_data *priv,
...
static inline int tz_get_target_freq(struct devfreq *devfreq, unsigned long *freq,
				u32 *flag)
{
	...
	int result = devfreq->profile->get_dev_status(devfreq->dev.parent, &stats);
	if (result) {
...
static inline int tz_notify(struct notifier_block *nb, unsigned long type, void *devp)
{
	int result = 0;
	struct devfreq *devfreq;
...
static inline int tz_start(struct devfreq *devfreq)
{
	...
	int ret = tz_init(priv, tz_pwrlevels, sizeof(tz_pwrlevels), &version,
				sizeof(version));
...
static inline int tz_stop(struct devfreq *devfreq)
{
	...
	for (int i = 0; adreno_tz_attr_list[i] != NULL; i++)
...
static inline int tz_suspend(struct devfreq *devfreq)
{
	struct devfreq_msm_adreno_tz_data *priv = devfreq->data;
...
static inline int tz_handler(struct devfreq *devfreq, unsigned int event, void *data)
{
	int result;
...
static inline void _do_partner_event(struct work_struct *work, unsigned int event)
{
...
static inline void do_partner_start_event(struct work_struct *work)
{
	_do_partner_event(work, DEVFREQ_GOV_START);
}

static inline void do_partner_stop_event(struct work_struct *work)
{
	_do_partner_event(work, DEVFREQ_GOV_STOP);
}

static inline void do_partner_suspend_event(struct work_struct *work)
{
	_do_partner_event(work, DEVFREQ_GOV_SUSPEND);
}

static inline void do_partner_resume_event(struct work_struct *work)
{
